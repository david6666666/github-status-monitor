...
├── stats_chart_vllm_omni.svg         # generated vLLM-Omni chart
├── stats_dashboard_vllm_omni.html   # generated vLLM-Omni dashboard
...
├── stats_chart_vllm_gr.svg           # generated vllm-gr chart
└── stats_dashboard_vllm_gr.html      # generated vllm-gr dashboard
```
...
3. `MONTHLY_SCENE_CONFIGS` groups the six `afd-plugin` people and ten `AgentInfer` people from the team sheet by 投入场景, and tracks five Beijing-based contributors in the separate `vllm-gr` scene. `afd-plugin` covers `vllm-project/afd-plugin`, `vllm-project/vllm`, and `vllm-project/vllm-ascend`; `AgentInfer` covers `openJiuwen-ai/agent-infer`, `vllm-project/router`, `vllm-project/semantic-router`, `vllm-project/vllm`, and `vllm-project/vllm-ascend`; `vllm-gr` covers `JiusiServe/vllm-gr`.
...
5. All-time PR counts use GitHub's complete search counts. Search requests and paginated results are paced globally to avoid GitHub Search API 403s. The monitor loads up to 10 recent PRs per contributor, repo, and state as candidates, then fetches code deltas only for the latest 80 displayed PRs. This keeps large-repository searches bounded while showing the repo beside every recent PR.
