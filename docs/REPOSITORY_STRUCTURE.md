...
4. Each scene uses two Beijing-time windows: the previous calendar month and the current calendar month through the latest run. Review candidates are searched by tracked reviewer, then each review timestamp is checked against the exact window. Raw commit, review, and code-churn metrics are summed across that scene's repos before scoring, so shared repos are combined before the 20% commit + 35% review + 45% code-churn score is calculated.
